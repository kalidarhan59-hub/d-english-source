...
export function SpeakEnglishButton({ text, className, label = "Озвучить по-английски", showLabel = false }: { text: string; className?: string; label?: string; showLabel?: boolean }) {
  ...
  const speak = () => {
    const result = beginEnglishSpeech(englishText, window.speechSynthesis, (value) => new SpeechSynthesisUtterance(value));
    if (result.status === "unsupported") {
      toast.error("Озвучивание недоступно в этом браузере. Английский текст остаётся видимым.");
      return;
    }
    ...
    utterance.onerror = () => { setPlaying(false); toast.error("Не удалось воспроизвести озвучку. Прочитай текст и попробуй снова."); };
  };
